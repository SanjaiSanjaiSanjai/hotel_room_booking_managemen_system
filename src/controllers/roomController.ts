...
export async function getRooms(req: Request, res: Response) {
    try {
        ...
        validateCondition(getRooms, MESSAGES.ROOMS_NOT_AVAILABLE)
...
export async function updateRoomDetails(id: number, isActive: boolean | undefined) {
    const customer_active = isActive === false
    const updateRoom = await roomRepository.update({ id: id }, { is_available: customer_active ? true : false })
    const getRoom = await roomRepository.findOne({ where: { id: id } })
    // console.log("updateRooms: ", updateRoom);
