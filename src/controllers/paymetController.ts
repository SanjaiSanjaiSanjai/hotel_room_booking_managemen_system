...
export async function payment(req: Request, res: Response) {
    try {
        ...
        validateCondition(IS_BOOKED, MESSAGES.NO_BOOKING)
        ...
        validateCondition(IS_PAYMENT_STATUS, MESSAGES.PAYMENT_STATUS_TYPE)
        validateCondition(IS_PAYMENT_METHOD, MESSAGES.PAYMENT_METHOD)
        const PAID: Payments | null = await paymentRepository.create({
            amount: amount,
            payment_status: payment_status,
            payment_method: payment_method,
            bookings: IS_BOOKED || undefined,
            customers: IS_CUSTOMER || undefined
        })
        const savedPayment: Payments | null = await paymentRepository.save(PAID)
        return successResponse(res, savedPayment, MESSAGES.DATA_SUCCESS)
    } catch (error) {
